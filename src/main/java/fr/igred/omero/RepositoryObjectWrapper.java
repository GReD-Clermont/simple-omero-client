...
public abstract class RepositoryObjectWrapper<T extends DataObject> extends AnnotatableWrapper<T> {
     ...
     */
    protected static boolean importImages(DataManager client, DataObject target, int threads, String... paths)
     ...
     * @throws IOException      Cannot read file.
     */
    protected static List<Long> importImage(DataManager client, DataObject target, String path)
    throws ServiceException, AccessException, IOException {
        ImportConfig config = new ImportConfig();
        String       type   = PojoMapper.getGraphType(target.getClass());
        config.target.set(type + ":" + target.getId());
    ...
    public void copyAnnotationLinks(DataManager client, RepositoryObjectWrapper<?> object)
