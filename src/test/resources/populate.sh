...
} 1> "$WORKDIR/image1.fake.ini" 2> /dev/null
...
touch "$WORKDIR/image2.fake"
{
    ...
    echo "sizeZ=2"
    echo "sizeT=7"
} 1> "$WORKDIR/image2.fake.ini" 2> /dev/null
...
touch "$WORKDIR/screen1.fake"
{
    ...
    echo "plateCols=3"
    echo "fields=2"
} 1> "$WORKDIR/screen1.fake.ini" 2> /dev/null
...
touch "$WORKDIR/screen2.fake"
{
    ...
    echo "plateCols=2"
    echo "fields=2"
} 1> "$WORKDIR/screen2.fake.ini" 2> /dev/null
...
omero login testUser3@localhost -w password3 -g testGroup2
PROJECT3=$(omero obj new Project name=TestProject3)
DATASET5=$(omero obj new Dataset name=TestDataset5)
omero obj new ProjectDatasetLink parent="$PROJECT3" child="$DATASET5"
omero import --output=ids "$WORKDIR/image1.fake" -T "$DATASET5"
omero quit
